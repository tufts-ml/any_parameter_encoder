...
if __name__ == "__main__":
    ...
    # Note: can only run one at a time due to global param store
        ...
        non_toy_bars = NonToyBarsDataset(
            ...
            num_models=1, num_docs=50, avg_num_words=5000, **data_config)
    ...
    for name, inference in zip(names, inferences):
        ...
        scale = pyro.get_param_store().get_param('decoder$$$scale')
        print(z_locs)
        print(z_scales)
        print(scale)
        ...
        np.save(trace_filename.replace('.npy', '_scale_param.npy'), np.array(scale.cpu().detach().numpy()))
