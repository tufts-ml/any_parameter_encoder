...
# Toy bars
results_dir = 'experiments/vae_experiments/final2'
results_csv = 'results.csv'
...
# datasets = load_toy_bars('toy_bars_100x100_18_topics')
# dataset_names = ['train', 'valid', 'test', 'test_single', 'test_double', 'test_triple']
...
results_dir = 'experiments/vae_experiments/final2'
dataset_names = ['train', 'valid', 'test']
train = np.load(os.path.join(results_dir, 'train_documents.npy'))
valid = np.load(os.path.join(results_dir, 'valid_documents.npy'))
datasets = [train[:15] * 10, valid[:15] * 10, valid[:15] * 10]
