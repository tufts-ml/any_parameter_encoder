...
if __name__ == "__main__":
    ...
    topics = torch.from_numpy(np.repeat(theta_KV, 5, axis=0)).type(torch.FloatTensor)
    print(topics.shape)
    print(avi.evaluate_loss(docs, topics) / x_DV.sum())
    pi_DK, var = vae.encoder.forward(x, topics)
    print('var', var)
    pi_DK = pi_DK.detach().numpy()
    sigma = torch.sqrt(var).detach().numpy()
