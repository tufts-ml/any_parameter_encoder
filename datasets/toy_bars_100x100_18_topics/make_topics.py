...
# bottommost
for i in range(5, 10):
    horizontal_topic = np.zeros((100, 100))
    horizontal_topic[i * 10: 10 + i * 10, :] = 1
    train_topics.append(horizontal_topic)

# leftmost
for j in range(1, 5):
    vertical_topic = np.zeros((100, 100))
    vertical_topic[:, j * 10: 10 + j* 10] = 1
...
for i in range(1, 5):
    # all pairs
    for j in range(1, 10):
        topic = np.zeros((100, 100))
        topic[i * 10: 10 + i * 10, :] += 1
        topic[:, j * 10: 10 + j* 10] += 1
