...
results_dir='experiments/r2'
find_lr=False
evaluate_svi_convergence=False
evaluate_svi_convergence_with_vae_init=False
run_train=True
evaluate=False
run_mcmc=False
use_cached=True
mdreviews=False
additive_topics=True
...
# toy bars data
if use_cached and os.path.exists(os.path.join(results_dir, 'train_topics.npy')):
...
elif use_cached and os.path.exists(os.path.join('experiments', 'train_topics.npy')):
...
if evaluate_svi_convergence_with_vae_init:
    vae = VAE_pyro(**model_config)
    data, topics = unzip_X_and_topics(next(datasets))
    data = torch.from_numpy(data.astype(np.float32))
    topics = torch.from_numpy(topics.astype(np.float32))
